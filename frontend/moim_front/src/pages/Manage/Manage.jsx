    ...
    return (
        <aside className="flex flex-col w-[30%] max-md:ml-0 max-md:w-full">
            <div className="flex flex-col grow items-start self-stretch py-4 pr-3 pl-16 max-md:pl-5 max-md:mt-6">
                ...
                <SidebarElementLink icon={<ArticleIcon />} text="모임 관리" color={activeSection === 'overview' ? "text-red-600" : "text-gray-400"} className="mt-16 max-md:mt-10" onClick={() => setActiveSection('overview')} />
                <SidebarElementLink icon={<PersonIcon />} text="프로필 설정" color={activeSection === 'profile' ? "text-red-600" : "text-gray-400"} className="mt-2.5" onClick={() => setActiveSection('profile')} />
            </div>
        </aside>
    );
}
        ...
        <div className="justify-between bg-white h-screen flex flex-col">
            <div className="flex-1">
                <div className="flex gap-5 h-full w-full max-md:flex-col max-md:gap-0">
                    ...
                    {activeSection === 'overview' ? <OverviewSection /> : <ProfileSection />}
                </div>
            </div>
        </div>
    );
